...
Runs `vite` in `development` mode using `db.json` as data source. `NODE_ENV` is set to `development` by default.
{spacer-2}
...
Serves test build using `db.json` as data source.
{spacer-2}
