...
The root folder contains link:sample.env.development[sample.env.*^] files for common environments with predefined effective variables. Remove the `sample` part of the filename so that https://vitejs.dev/[Vite] automatically inserts the variables into the `import.meta.env` object by running mode.